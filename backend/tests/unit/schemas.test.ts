...
import { createSchema as txCreateSchema, filterSchema } from "../../src/schemas/transaction_schema";
...
describe("user_schema", () => {
	describe("createSchema", () => {
		...
		it("should reject short first_name", () => {
			expect(() => createSchema.parse({ ...validUser, first_name: "J" })).toThrow();
		});

		it("should reject invalid email", () => {
			expect(() => createSchema.parse({ ...validUser, email: "invalid" })).toThrow();
		});

		it("should reject weak password (no uppercase)", () => {
			expect(() => createSchema.parse({ ...validUser, password: "password1" })).toThrow();
		});

		it("should reject weak password (no digit)", () => {
			expect(() => createSchema.parse({ ...validUser, password: "Password" })).toThrow();
		});

		it("should reject short password", () => {
			expect(() => createSchema.parse({ ...validUser, password: "Pa1" })).toThrow();
		});

		it("should accept valid French phone number", () => {
			expect(() => createSchema.parse({ ...validUser, phone_number: "0612345678" })).not.toThrow();
		});

		it("should reject invalid phone number", () => {
			expect(() => createSchema.parse({ ...validUser, phone_number: "123" })).toThrow();
		});
	});
...
describe("authentication_schema", () => {
	it("should accept valid login", () => {
		expect(() => login.parse({ email: "test@test.com", password: "Password1" })).not.toThrow();
	});
	...
	it("should accept valid reset password", () => {
		expect(() => resetPassword.parse({ token: "abc", password: "Password1" })).not.toThrow();
	});
});

describe("cash_register_schema", () => {
	it("should accept valid close data", () => {
		expect(() => closeSchema.parse({
