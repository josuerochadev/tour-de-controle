...
const EditUser: React.FC = () => {
	...
	const handleFieldChange = (field: string, value: unknown) => {
		const updated = { ...formData, [field]: value } as UserFormData;
		setFormData(updated);
		if (errors[field] !== undefined) {
			const result = updateUserSchema.safeParse(updated);
		...
		<div className="max-w-[720px] mx-auto">
			...
			<form onSubmit={handleSubmit} className="bg-paper-soft border border-sand rounded-3xl p-8 space-y-5">
				...
				<div>
					...
					<input id="postal_address" type="text" value={formData.postal_address} onChange={(e) => handleFieldChange("postal_address", e.target.value)} className={inputClass} />
				</div>
				<div>
					<label htmlFor="phone_number" className={labelClass}>Telephone</label>
					<input id="phone_number" type="tel" value={formData.phone_number} onChange={(e) => handleFieldChange("phone_number", e.target.value)} className={inputClass} />
				...
				<div>
					...
					<select id="id_role" value={formData.id_role} onChange={(e) => handleFieldChange("id_role", Number.parseInt(e.target.value))} className={inputClass} required>
