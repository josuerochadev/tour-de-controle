	...
	const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			if (success) {
				...
				if (user) {
					console.log("✅ Utilisateur valide, redirection en cours...");
					resetForm();
					navigate("/dashboard");
				} else {
					console.error(
						"🚨 Impossible de récupérer l'utilisateur après connexion.",
					);
			...
			<div className="bg-white rounded-lg shadow-lg p-8 w-full max-w-md">
				...
				<form onSubmit={handleSubmit} className="space-y-6">
					...
					<div>
						...
						<div className="relative">
							<input
								...
								className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 pr-10"
					...
					<button
						...
						disabled={loading}
						className="w-full bg-cyan-600 text-white rounded-md py-2 font-medium hover:bg-cyan-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-500 disabled:opacity-50 disabled:cursor-not-allowed"
					>
				...
				<div className="mt-8 text-sm text-gray-600 text-center">
					...
					<p className="mt-2 text-xs">
						Besoin d'aide ? Votre identifiant est votre adresse e-mail
						enregistrée.
					</p>
				</div>
			</div>
